...
class PlaylistSoundPlayer extends Component {
  ...
  renderTrackList() {
    ...
    const tracks = playlist.tracks.map((track, i) => {
      ...
      return (
        ...
        >
          <span className="flex-auto semibold white">{track.user.username} - {track.title}</span>
          <span className="clock">{Timer.prettyTime(track.duration / 1000)}</span>
        </button>
      );
    });
  ...
  render() {
    ...
    return (
      <div className="bg-darken-1 green mt1 mb3 rounded mh42vh">
        <div className="p2">
          ...
          <h2 className="h2 nowrap caps mt0 mb2 semibold white">{playlist ? playlist.title : ''}</h2>
